...
while True:
    for ticker in tickers:
        if upbit.get_balance(ticker) != 0 :   
                if upbit.get_avg_buy_price(ticker)*0.85 > pyupbit.get_current_price(ticker): # 2. 해당종목을 보유면서, 현재 가격이 구매 가격보다 15% 낮은 경우,     
                ...
                elif upbit.get_avg_buy_price(ticker)*1.1 < pyupbit.get_current_price(ticker):# 1. 해당종목 현재 가격이 구매 가격보다 10% 높고, 보유분이 전체의 10% 이상일 경우에만 해당 코인 잔고의 5% 씩 판매 
                    if upbit.get_balance(ticker)*pyupbit.get_current_price(ticker) > (upbit.get_amount('ALL') + upbit.get_balance("KRW"))*0.1: 
